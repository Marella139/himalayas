...
if(isset($_REQUEST['btnUpdateNavigation']))
	{
		...
		$id=$_REQUEST['nav_id'];
		$update_navigation="UPDATE navigation SET navID='$id' , navName='$name'
		WHERE 'navID'='$id' ";
		$name=$_REQUEST['nav'];
...
if(isset($_REQUEST['btnUpdateAbout']))
	{
		...
		$plus=$_REQUEST['gplus'];
		$out=$_REQUEST['out'];
		$update="UPDATE aboutme SET aboutText='$text',aboutFB='$fb',aboutG='$plus', aboutOut='$out' WHERE aboutID=".$id;
		mysqli_query($konekcija,$update);
		header('Location: ' . $_SERVER['HTTP_REFERER']);
		exit;
	}
...
if(isset($_REQUEST['btnUpdateHistory']))
	{
		...
		$update="UPDATE history SET hi_prvi='$prvi', hi_drugi='$drugi', 
		hi_treci='$treci', hi_naslov='$naslov' WHERE hi_id=".$id;
		mysqli_query($konekcija,$update);
		header('Location:' . $_SERVER['HTTP_REFERER']);
		exit;
	}
...
if(isset($_REQUEST['btnUpdateFacts']))
	{
		...
		$update="UPDATE facts SET row_jedan='$prva',row_dva='$druga',
		row_tri='$treca',row_jedan_naslov='$prvi',row_dva_naslov='$drugi',row_tri_naslov='$treci' WHERE fact_id=".$id;
		mysqli_query($konekcija,$update);
		header('Location: ' . $_SERVER['HTTP_REFERER']);
		exit;
	}
